...
class Database:
    ...
    def UpdateRow(self, table_name, col_name, set_value, row_id):
        try:
            ...
            sql_query = f"update {table_name} set {col_name} = "
            if type(set_value) == int:
                sql_query += set_value
            else:
                sql_query += f"'{set_value}'"
            sql_query += f"where {primary_key_name} = "
            if type(row_id) == int:
                sql_query += row_id
            else:
                sql_query += f"'{row_id}'"
    ...
    def DeleteRow(self, table_name, row_id):
        try:
            self.cursor.reset()
            primary_key_name =  self.__FindPrimaryKey(table_name)
            sql_query = f"delete from {table_name} where {primary_key_name} = "
            if type(row_id) == int:
                sql_query += row_id
            else:
                sql_query += f"'{row_id}'"
...
print(database.SelectTable("Arma"))
print(database.InsertRow("Arma", ("Guinsoo", "Espada", 100, 150, "Lendária")))
print(database.UpdateRow("Arma", "Tipo", "Arco", "Guinsoo"))
print(database.SelectTable("Arma"))
print(database.DeleteRow("Arma", "Guinsoo"))
database.Close()